...
class FaceEmbedderMovidiusFacenet:
    ...
    def __init__(self, movidius_id=0):
        ...
        self.graph_fifo_in, self.graph_fifo_out = self.graph.allocate_with_fifos(self.device, graph_in_memory)
    ...
    def get_embedding_mem(self, cv_image):
        ...
        self.graph.queue_inference_with_fifo_elem(self.graph_fifo_in, self.graph_fifo_out, image.astype(np.float32), None)

        # Get results from NCS
        #output, userobj = self.graph.GetResult()
        output, userobj = self.graph_fifo_out.read_elem()
