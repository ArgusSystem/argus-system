...
class Predictor:
    ...
    def load_face_db(self, face_db_path):
        ...
        for path in os.listdir(face_db_path):
            ...
            if imgs is None or len(imgs) > 1:
                print('No face found in image %s, skipped' % image_path)
                continue
    ...
    def recognition(self, img):
        ...
        print('face detection time: %dms' % int((time.time() - s) * 1000))
        if imgs is None:
            return None, None, None
        #cv2.imshow("face", imgs[0])
        #cv2.waitKey(1)
        imgs = self.process(imgs)
        imgs = np.array(imgs, dtype='float32')
        s = time.time()
        features = self.infer(imgs)
        recognition_time = int((time.time() - s) * 1000)
        print('face recognition time: %dms' % int((time.time() - s) * 1000))
        names = []
        probs = []
        for i in range(len(features)):
            ...
            if prob > self.threshold:
                ...
                names.append('unknow')
        return boxes, names, recognition_time
        ...
        if boxes_c is not None:
            for i in range(boxes_c.shape[0]):
                ...
                cv2.rectangle(img, (corpbbox[0], corpbbox[1]),
                              (corpbbox[2], corpbbox[3]), (255, 0, 0), 5)
                # 判别为人脸的名字
                img = self.add_text(img, name, corpbbox[0], corpbbox[1] -40, color=(255, 0, 0), size=40)
        return img
...
if __name__ == '__main__':
    ...
    total_recognized_frames = 0
    while True:
        ret, img = cap.read()
        if ret:
            start = time.time()
            boxes, names, recognition_time = predictor.recognition(img)
            if boxes is not None:
                img = predictor.draw_face(img, boxes, names)
                print('predicted face location: ', boxes.astype('int32').tolist())
                print('face name:', names)
                print('total time: %dm' % int((time.time() - start) * 1000))
                total_recognition_time += recognition_time
                total_recognized_frames += 1
    ...
    print("average recognition time: " + str(total_recognition_time / total_recognized_frames))
