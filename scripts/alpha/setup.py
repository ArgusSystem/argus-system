...
def create_rule(type_who, value_who, rule_areas, start_time, end_time, severity, warden):
    ...
    RestrictionWarden.insert(restriction_id=restriction_id, role_id=warden).execute()


def setup(faces_directory):
    ...
    def create_person(wanted_id, person_name, role_id):
        ...
        for photo in listdir(path.join(faces_directory, person_name)):
            photo_key = person.next_photo_key(photo)
            PersonPhoto.insert(person=person_id, filename=photo_key, preprocessed=False).execute()
            people_storage.store(name=photo_key, filepath=path.join(faces_directory, person_name, photo))
    ...
    create_rule('role', [guest_role_id], [private_area_type_id], '00:00', '23:59', warning_severity, host_role_id)
    create_rule('unknown', None, [public_area_type_id], '00:00', '23:59', warning_severity, host_role_id)
    create_rule('unknown', None, [private_area_type_id], '00:00', '23:59', critical_severity, host_role_id)
